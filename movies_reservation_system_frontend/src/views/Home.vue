  ...
  <v-app>
    ...
    <v-row>
      <v-card
        ...
        outlined
        shaped
        style="margin: 50px 20px 50px 30px"
      >
        <v-img
          height="300"
          width="300"
        ...
        <v-card-text>
          <v-chip-group active-class="black accent-4 white--text" column>
            <v-chip style="font-size: 0.8rem"
              >{{ movie.date }}: From {{ movie.start_time }} to
              {{ movie.end_time }}</v-chip
            >
          </v-chip-group>
        </v-card-text>

        <v-card-actions>
          <v-btn color="#4A646C" dark @click="reserve(movie.id)">
            Reserve
          </v-btn>
          <v-btn
            v-if="manager"
...
export default {
  ...
  created() {
    axios
      .get("http://127.0.0.1:8000/getAllMovies", {})
      .then((response) => {
